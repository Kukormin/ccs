...
foreach($arResult['ORDER_BY_STATUS'] as &$group) {
    foreach ($group as $k => &$order) {
        ...
        foreach ($order["BASKET_ITEMS"] as &$item) {
            ...
            if (isset($items[$mxResult['ID']])) {
                $item['IMAGE'] = $parents[$mxResult['ID']];
                continue;
            }
            $items[$mxResult['ID']] = true;
            $arFilter = Array("IBLOCK_ID" => $mxResult['IBLOCK_ID'], "ID" => $mxResult['ID']);
            $res = CIBlockElement::GetList(Array(), $arFilter);
            while ($ob = $res->GetNextElement()) {
                $arFields = $ob->GetFields();
            }
            
            $parents[$mxResult['ID']] = $arFields;
        }
    }
}
