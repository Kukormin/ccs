            ...
            <div class="b-slider__item" id="<? echo $strMainID; ?>">
                <div class="b-mod__item b-mod__item-about-novelty">
                    ...
                    <div class="b-mod__item-price">
                        <? if($arItem['MIN_PRICE']['DISCOUNT_VALUE'] !== NULL) {

                                if($arItem['MIN_PRICE']['DISCOUNT_VALUE'] == $arItem['MIN_PRICE']['VALUE']){
                                    echo number_format($arItem['MIN_PRICE']['DISCOUNT_VALUE'], 0, '', ' '); 
                                }else{
                                    echo "<span style=\"text-decoration: line-through; color:#aaa;\">" .number_format($arItem['MIN_PRICE']['VALUE'], 0, '', ' ') . "</span>" . ' ' .
                                                    number_format($arItem['MIN_PRICE']['DISCOUNT_VALUE'], 0, '', ' ');
                            }

                            
                        } 
                            elseif(is_string($arItem['CATALOG_PRICE_1'])) {echo number_format($arItem['CATALOG_PRICE_1'], 0, '', ' ');}elseif($arItem['PRICE']){echo number_format($arItem['PRICE'], 0, '', ' ');} ?> <span class="rub">i</span>
                    ...
                    <? } elseif(isset($arItem['SKU_NUM'])) {
                            echo '<span>/ '.$arItem['SKU_NUM'].' шт</span>';
                    } ?>
                    </div>
