...
foreach($main_section as $ids) {
    ...
    $sku_number = [];
    if($arIB) {
        $result = CIBlockElement::GetList(Array(), Array('ACTIVE' => 'Y',"IBLOCK_ID" => $iblock_id), false, false, Array('ID', 'PROPERTY_NUMBER'));
        while($arr = $result->GetNext())
        {
            $el_id[] = $arr['ID'];
            $sku_number[$arr['ID']] = $arr['PROPERTY_NUMBER_VALUE'];
        }
        ...
        while ($ar_res = $db_res->Fetch())
        {
            ...
            $prices[$ar_res['PRODUCT_ID']][] = $sku_number[$ar_res['PRODUCT_ID']];
        }
    }

    $res = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array('ACTIVE' => 'Y',"IBLOCK_ID" => $ids), false, false, Array("ID", "NAME", "IBLOCK_ID","PREVIEW_PICTURE", "IBLOCK_NAME", "DETAIL_PICTURE", "DATE_CREATE", "CATALOG_GROUP_1", "OF_IBLOCK_ID", "DETAIL_PAGE_URL", "SORT"));
    while($ar = $res->GetNext())
    {
        ...
        $ar['SKU_NUM'] = isset($num_ib[0]) ? $num_ib[0] : $num_ib[1];
        $arResult['ITEMS'][] = $ar;
        $product_ids[] = $ar['ID'];
    }
}
