<?
/*Слегка модифицированная версия экспорта в YML заточенная под мэйлсру*/
...
if (empty($arRunErrors))
{
	...
	fwrite($fp, '<torg_price date="'.date("Y-m-d H:i").'">'."\n");
	fwrite($fp, '<shop>'."\n");

	fwrite($fp, '<shopname>'.$APPLICATION->ConvertCharset(htmlspecialcharsbx(COption::GetOptionString('main', 'site_name', '')), LANG_CHARSET, 'windows-1251')."</shopname>\n");
