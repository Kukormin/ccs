...
class ICrmOrderActions
{
    ...
    public static function uploadOrders($pSize = 50, $failed = false, $orderList = false) {
        ...
        $api = new RestApiExt($api_host, $api_key);
    ...
    public static function orderCreate($arFields, $api, $arParams, $send = false, $site = null) {
        ...
        $api_host = COption::GetOptionString(self::$MODULE_ID, self::$CRM_API_HOST_OPTION, 0);
        $api_key = COption::GetOptionString(self::$MODULE_ID, self::$CRM_API_KEY_OPTION, 0);
        $apiv4 = new RestApiExt($api_host, $api_key);
        ...
        $order = array(
            'number'          => $arFields['ACCOUNT_NUMBER'] . '-tim',
            'externalId'      => $arFields['ID'] . '-tim',
        ...
        while ($ar = $rsOrderProps->Fetch()) {
            if ($search = array_search($ar['CODE'], $arParams['optionsLegalDetails'][$arFields['PERSON_TYPE_ID']])) {
                $order['contragent'][$search] = $ar['VALUE'];
        ...
        while ($p = $rsOrderBasket->Fetch()) {
            ...
            /**
             * пытаемся определить, применялась ли скидка к товару
             */

            // если не применялся купон, но применилась скидка
            if (! $p['DISCOUNT_COUPON'] && $p['DISCOUNT_VALUE'] && $p['DISCOUNT_NAME']) {
                // находим id примененной скидки
                $matches = [];
                preg_match('/^\[(?:(\d+))\]/', $p['DISCOUNT_NAME'], $matches);
                if (! empty($matches) && isset($matches[1]) && !empty($matches[1])) {
                    $item['priceType']['code'] = $matches[1];
                    $discountValue = (int)$p['DISCOUNT_VALUE'];
                    // сбрасываем скидку и цену
                    unset($item['initialPrice']);
                    unset($item['discount']);
                }
        ...
        if (isset($_SESSION['retailcrm']['utm_source'])) {
            $order['source']['source'] = $_SESSION['retailcrm']['utm_source'];
            $order['customFields']['usource'] = $_SESSION['retailcrm']['utm_source'];
        }
        if (isset($_SESSION['retailcrm']['utm_medium'])){
            $order['source']['medium'] = $_SESSION['retailcrm']['utm_medium'];
            $order['customFields']['umedium'] = $_SESSION['retailcrm']['utm_medium'];
        }
        if (isset($_SESSION['retailcrm']['utm_campaign'])) {
            $order['source']['campaign'] = $_SESSION['retailcrm']['utm_campaign'];
            $order['customFields']['ucampaign'] = $_SESSION['retailcrm']['utm_campaign'];
        }
        if (isset($_SESSION['retailcrm']['utm_term'])){
            $order['customFields']['uterm'] = $_SESSION['retailcrm']['utm_term'];
        }
        if (isset($_SESSION['retailcrm']['utm_content'])) {
            $order['customFields']['ucontent'] = $_SESSION['retailcrm']['utm_content'];
        }
        ...
        if($send) {
            ...
            $orderInCRM = false;
            try {
                $orderInCRM = $api->ordersGet($order['externalId']);
                $orderInCRM = $orderInCRM->offsetGet('success');
            ...
            if ($orderInCRM) {
                if ($orderEditV4 = self::apiMethod($apiv4, 'orderEdit', __METHOD__, $order, $site)) {
                    return $orderEditV4;
                } else {
                    return false;
                }
            } else {
                if ($orderEditV4 = self::apiMethod($apiv4, 'ordersCreate', __METHOD__, $order, $site)) {
                    return $orderEditV4;
                } else {
                    return false;
                }
    ...
    public static function orderHistory() {
        ...
        try {
            ...
            $orderHistoryV4 = $apiV4->orderHistory($dateStart);
        ...
        foreach ($orderHistory as $order) {
            ...
            if (isset($order['externalId']) && $order['externalId']) {
                ...
                $orderV4 = $apiV4->ordersGet($order['id'], 'id');
                $orderV4 = $orderV4['order'];
                time_nanosleep(0, 250000000);
                ...
                foreach($order['items'] as $item) {
                    ...
                    if ((isset($item['created']) && $item['created'] == true) || (isset($order['created']) && $order['created'] == true)) {
                    ...
                    // определяем тип цены
                    foreach ($orderV4['items'] as $itemV4) {
                        
                        if ($itemV4['offer']['externalId'] != $item['offer']['externalId']) {
                            continue;
                        }
                        if ($itemV4['priceType']) {
                            $discountId = (int)$itemV4['priceType']['code'];
                            break;
                        }
                    }

                    if ($discountId && ($discount = CCatalogDiscount::GetByID($discountId))) {
                        $arProduct['DISCOUNT_NAME'] = '[' . $discount['ID'] . ']' . ' ' . $discount['NAME'];
                        $arProduct['DISCOUNT_VALUE'] = ($discount['VALUE_TYPE'] == 'P')
                            ? (int)$discount['VALUE'] . '%'
                            : $discount['VALUE'];
                        $arProduct['DISCOUNT_COUPON'] = NULL;
                        $fullPrice = ($discount['VALUE_TYPE'] == 'P')
                            ? $arProduct['PRICE'] / (1 - $discount['VALUE']/100)
                            : $arProduct['PRICE'] + $discount['VALUE'];

                        $arProduct['DISCOUNT_PRICE'] = $fullPrice - $arProduct['PRICE'];
                    }
...
class RestApiExt extends \RetailCrm\RestApi {
    const VERSION = 'v4';
    ...
    public function orderHistory(
        ...
        if ($startDate) {
            $parameters['filter']['startDate'] = $this->ensureDateTime($startDate);
        }
        if ($endDate) {
            $parameters['filter']['endDate'] = $this->ensureDateTime($endDate);
        }
