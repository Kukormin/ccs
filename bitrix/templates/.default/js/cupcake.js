$(document).ready(function() {
	...
	(function () {
		$(".slider-nav-gal a").click(function(){
			...
			$(".slider-for").append("<img src='" +path + "' />").find("img").css({'position':'absolute', 'z-index': 1, 'opacity': 0}).bind("load",function(){
...
/*popup	*/
	...
	$('body').on('click', '.js-modal-tobasket', function (e) {
		...
		$.post('/include/ajax_basket.php', {'ajaxaction': 'dupadd', 'ajaxaddid': addid, 'quantity': parseInt($(".js-modal-counter",modal).text()), 'params': params},function (resp) {
			if (resp != 0) {
	...
	$('.js-basket-option').on('change', function () {
		...
		var bid = line.data('oiid');
		var opts = {'action_var': basket_settings.action_var, 
			'basketItemId': bid,
	...
	$(".js-duplicate-item").on('click', function (e) {
		...
		$('.js-basket-option, .js-basket-prop', line).each(function () {
			params[$(this).data('code')] = {'VALUE': $(this).children('option:selected').text(), 'NAME': $(this).data('name'),};
		});
		$.post('/include/ajax_basket.php', {'ajaxaction': 'dupadd', 'ajaxaddid': id, 'params': params},function (resp) {
