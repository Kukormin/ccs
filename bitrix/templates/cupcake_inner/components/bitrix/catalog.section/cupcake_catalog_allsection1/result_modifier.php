...
foreach($main_section as $ids) {
    ...
    if($arIB) {
        ...
        $result = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array('ACTIVE' => 'Y', "IBLOCK_ID" => $iblock_id), false, false, Array());
    ...
    while($ar = $res->GetNext())
    {
        ...
        $ar["PRICES"] = CIBlockPriceTools::GetItemPrices($arParams["IBLOCK_ID"], $arResult["PRICES"], $ar, $arParams['PRICE_VAT_INCLUDE'], $arConvertParams);
            $ar['MIN_PRICE'] = '';
        if (!empty($ar['PRICES'])) {
            $ar['MIN_PRICE'] = CIBlockPriceTools::getMinPriceFromList($ar['PRICES']);
        } else {
            $price_ib = array();
            ...
            foreach ($sku as $sku_items) {
                foreach ($sku_items as $id=>$n) {
                    if ($prices[$id][0])
                    $price_ib[] = $prices[$id][0];
                }
            }
            unset($sku);
            unset($sku_items);
            $ar['MIN_PRICE'] = array('VALUE'=>reset($price_ib));
        }
        $arResult['ITEMS'][] = $ar;
        $product_ids[] = $ar['ID'];
    }
