...
foreach($main_section as $ids) {
    ...
    $res = CIBlockElement::GetList(Array(), Array('ACTIVE' => 'Y',"IBLOCK_ID" => $ids), false, false, Array("ID", "NAME", "IBLOCK_ID","PREVIEW_PICTURE", "IBLOCK_NAME", "DETAIL_PICTURE", "DATE_CREATE", "CATALOG_GROUP_1", "OF_IBLOCK_ID"));
...
usort($arResult['ITEMS'], function ($a, $b) {
    $a1 = strtotime($a['DATE_CREATE']);
    $b1 = strtotime($b['DATE_CREATE']);
    if ($a1 == $b1) {
        return 0;
    }
    return ($a1 < $b1) ? 1 : -1;
...
$sliced_arr = array_slice($arResult['ITEMS'], 0, 30);
