...
//Фильтр начинок
$groups_res = CIBlockElement::GetElementGroups($arResult['ID']);
$groups = [];
while ($gr = $groups_res->GetNext()){
	$groups[] = $gr['ID'];
}
if (isset($arResult['PRODUCT_PROPERTIES']['FILLING'])) {
	foreach ($arResult['PRODUCT_PROPERTIES']['FILLING']['VALUES'] as $id=>$name) {

		$el = CIBlockElement::GetByID($id);
		$i = $el->GetNext();
		$el = CIBlockElement::GetProperty($i['IBLOCK_ID'],$id, array('sort'=>'asc'), array('CODE'=>'CATEGORIES'));
		$sections = array();
		while ($item = $el->Fetch()){
			$sections[$item['VALUE']] = $item['VALUE'];
		}
		$unset = true;
		if (isset($sections[$arResult['IBLOCK_SECTION_ID']])) {
			$unset = false;
		}else{
			foreach ($groups as $gid) {
				if (isset($sections[$gid])) {
					$unset = false;
					break;
				}	
			}
		...
		if ($unset){
			unset($arResult['PRODUCT_PROPERTIES']['FILLING']['VALUES'][$id]);
		}
	}
	
	if (!isset($arResult['PRODUCT_PROPERTIES']['FILLING']['VALUES'][$arResult['PRODUCT_PROPERTIES']['FILLING']['SELECTED']])){
		$arResult['PRODUCT_PROPERTIES']['FILLING']['SELECTED'] = key($arResult['PRODUCT_PROPERTIES']['FILLING']['VALUES']);
	}
}


// дополнительные товары
$db_list = CIBlockSection::GetList(array("SORT" => "ASC"), array("IBLOCK_ID" => $arResult['IBLOCK_ID'], 'ID' => $arResult["SECTION"]["ID"], 'ACTIVE' => 'Y'), true, Array("UF_ACSESS_BOUND"));
while($ar_result = $db_list->GetNext()):
$arResult['BOUND_ID'] = $ar_result['UF_ACSESS_BOUND'];
endwhile;

$arCompareList = array(); //для проверки, брать из корзины или нет
$groups = array();
foreach($arResult['BOUND_ID'] as $ids) {
    $arFilter = Array("IBLOCK_ID"=>16, "SECTION_ID"=>$ids, 'ACTIVE' => 'Y');
    $res = CIBlockSection::GetByID($ids);
    if($arRes = $res->GetNext()) {
        $arResult['BOUND_PRODUCT_ID'][$ids]['NAME'] = $arRes['NAME'];
    }
    $res = CIBlockElement::GetList(Array(), $arFilter);
    
    while ($ob = $res->GetNextElement()) {
        $arResult['BOUND'][$ids][$ob->fields['ID']]['BOUND_FIELDS'] = $ob->GetFields();
       
        $groups[$ob->fields['ID']] = $ids;
        $arResult['BOUND_PRODUCT_ID'][$ids][] = $ob->fields['ID'];
        $arCompareList[] = $ob->fields['ID'];
    }
    foreach($arResult['BOUND_PRODUCT_ID'][$ids] as $product_ids) {
        $price_res = CPrice::GetList(Array(), array("PRODUCT_ID" => $product_ids));
        while($obp = $price_res->Fetch()) {

            $arResult['BOUND'][$ids][$obp['PRODUCT_ID']]['BOUND_FIELDS']['PRICE'] = $obp['PRICE'];
        }
    }
}

// содержимое корзины по дополнительным товарам
CModule::IncludeModule('sale');
$resBasket = CSaleBasket::GetList(array(), array("FUSER_ID" => CSaleBasket::GetBasketUserID()));
$arResult['BOUND_BASKET'] = array();
while ($arBasketProd = $resBasket->GetNext()){
    if (in_array($arBasketProd['PRODUCT_ID'], $arCompareList)) {
        $arResult['BOUND_BASKET'][$groups[$arBasketProd['PRODUCT_ID']]][$arBasketProd['PRODUCT_ID']] = array(
            'ID' => $arBasketProd['ID'],
            'GID' => $groups[$arBasketProd['PRODUCT_ID']],
            'PRODUCT_ID' => $arBasketProd['PRODUCT_ID'],
            'QUANTITY' => $arBasketProd['QUANTITY'],
            'PRICE' => $arBasketProd['PRICE']
        );
    }
